...
describe("JsonStore", () => {
  ...
  it("migrates a v1 database on load, preserving existing agents/messages/runs", async () => {
    ...
    expect(data.credentialAuditEvents).toEqual([]);
  ...
  it("migrates a v3 database on load, adding empty credential tables", async () => {
    const root = await mkdtemp(path.join(tmpdir(), "launchpad-store-migrate-v3-test-"));
    temporaryDirectories.push(root);
    const dbPath = path.join(root, "db.json");
    const v3 = {
      version: 3,
    ...
    await writeFile(dbPath, JSON.stringify(v3, null, 2), "utf8");
